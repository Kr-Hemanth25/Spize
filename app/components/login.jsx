...
const  Login= ({isUser,user}) => {
  ...
  if (session||mlogin.isuser) {
    console.log("done by me bro")
    router.push('/home');
  }
  else{
    console.log("signout handeled by me login",mlogin)
    router.push('/login')}
  ...
  const onsubmit=async(data)=>{
    ...
    console.log(result.user,result.mission)
    if(result.user){
      reset()
  ...
  return (
    <div className="flex flex-col gap-[2vh] min-h-screen pt-[5vh] items-center text-white">
        ...
        <h1 className="text-2xl font-bold mb-0">Sign In</h1>
        {/* <h1 className='text-center text-gray-200 pb-[0vh]'>Check Mail,Otherwise Company Is not responsible For Anything</h1> */}
        ...
        <input
          ...
          />
          {errors.email && <span className="text-gray-200 capitalize text-sm text-center">{errors.email.message}</span>}
        <input
          type="password"
        ...
        />
          ...
          <Link href="/login/reset" className='text-center text-blue-300'>Forgot Password?</Link>
        ...
        <svg className="h-6 w-5 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 73 73" version="1.1">
        ...
        <p className="text-gray-400 text-sm mt-4">
          Dont have an account? <Link href="/signup" className="text-blue-500 hover:underline">Sign Up</Link>
        </p>
